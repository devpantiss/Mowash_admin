...
const ChartCardPair = ({ item, totalHouseholds }) => (
  <div
...
const Row10 = ({ village }) => {
  ...
  // Calculate percentages based on TotalHouseHoldsSurveyed
  const toiletsAtHomePercent = Math.round((village.ToiletsAtHome / totalHouseholds) * 100);
  const publicToiletAccessPercent = Math.round((village.PublicToiletAccess / totalHouseholds) * 100);
  const drainageConnectivityPercent = Math.round((village.DrainageConnectivity / totalHouseholds) * 100);
  const openDefecationPercent = Math.round((village.OpenDefacation / totalHouseholds) * 100);
  const bathroomAtHomePercent = Math.round((village.BathroomAtHome / totalHouseholds) * 100);

  // Combined data for charts and initiatives
  const dashboardData = [
    {
      category: "Toilets at Home",
      chartData: [toiletsAtHomePercent, 100 - toiletsAtHomePercent],
      labels: ["Have Toilets", "Need Toilets"],
      initiatives: [
        `Construct toilets for ~${village.ToiletsAtHome} no-toilet households via SBM-G.`,
    ...
    },
    {
      ...
      chartData: [drainageConnectivityPercent, 100 - drainageConnectivityPercent],
      labels: ["Connected", "Need Drainage"],
      initiatives: [
        `Develop septic tanks/open drains for ~${totalHouseholds - village.DrainageConnectivity} households.`,
    ...
    {
      category: "Bathrooms",
      chartData: [bathroomAtHomePercent, 100 - bathroomAtHomePercent],
      labels: ["Covered", "Need Bathrooms"],
      initiatives: [
        `Include bathrooms in PMAY-G houses for ~${totalHouseholds - village.BathroomAtHome} households.`,
    ...
    },
    {
      ...
      chartData: [publicToiletAccessPercent, 100 - publicToiletAccessPercent],
      labels: ["Covered", "Need CSCs"],
      initiatives: [
        `Build CSCs for ~${totalHouseholds - village.PublicToiletAccess} no-toilet households.`,
    ...
    },
    {
      ...
      chartData: [openDefecationPercent, 100 - openDefecationPercent],
      labels: ["Practice Open Defecation", "Do Not"],
      initiatives: [
        `Conduct behavior change campaigns (SBM-G) to reduce open defecation for ~${village.OpenDefacation} households.`,
          ...
          <Slider {...settings}>
            {dashboardData.map((item, index) => (
              <div key={index} className="px-4 py-4">
                <ChartCardPair item={item} totalHouseholds={totalHouseholds} />
              </div>
            ))}
          </Slider>
        </div>
