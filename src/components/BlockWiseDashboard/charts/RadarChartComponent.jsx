...
class CustomTooltip extends PureComponent {
  render() {
    const { active, payload, label, totalPopulation } = this.props;
    if (active && payload && payload.length) {
      const prevalence = payload[0].value;
      const actualValue = Math.round((prevalence / 100) * totalPopulation);
      return (
...
export default class RadarChartComponent extends PureComponent {
  ...
  render() {
    const { data, totalPopulation } = this.props;
      ...
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
          ...
          <Radar
            ...
            fillOpacity={0.6}
          />
          <Tooltip content={<CustomTooltip totalPopulation={totalPopulation} />} />
        </RadarChart>
      </ResponsiveContainer>
    );
  }
